...
export async function GET(request: NextRequest) {
  // --- 1. パラメータまたは時間からキーワードを決定 ---
  const { searchParams } = new URL(request.url);
  const manualKeyword = searchParams.get('keyword'); // URLに ?keyword=... があれば優先
  ...
  try {
    ...
    // より強力な「search」エンドポイントに変更
    const searchUrl = `https://games.roblox.com/v1/games/search?keyword=${encodeURIComponent(selectedKeyword)}&maxRows=30`;
    
    const searchRes = await fetch(searchUrl, { cache: 'no-store' });
    const searchData = await searchRes.json();

    // search APIは構造が少し違う場合があるので、dataがあるか確認
    if (searchData?.data && searchData.data.length > 0) {
      ...
      results.discovery_error = `Roblox Search API returned 0 results for "${selectedKeyword}".`;
    }
